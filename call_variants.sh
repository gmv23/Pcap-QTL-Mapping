...
grep ">" /workdir/gmv23/bsa/cpepo_genome/Cpepp_v4.1.chr.fa | sed 's/^.//' > $out_folder/chromosome_names.txt

# Make file with names of input bam files
ls $in_folder/*sorted.bam > $out_folder/sorted_bams.txt

# Make file with names of bcf files for each chromosome
cat $out_folder/chromosome_names.txt | sed 's/\./_/g' | sed 's/$/.bcf/' > $out_folder/bcf_chrom_files.txt

# Index input bam files in parallel
parallel samtools index -@ 4 {} :::: $out_folder/sorted_bams.txt

# Run mpileup for each chromosome in parallel#
parallel --link bcftools mpileup -A -q 20 -b $out_folder/sorted_bams.txt -a AD,INFO/AD \
-f /workdir/gmv23/bsa/cpepo_genome/Cpepp_v4.1.chr.fa \
-r {1} -o $out_folder/{2} --threads 3 -O b :::: $out_folder/chromosome_names.txt \
:::: $out_folder/bcf_chrom_files.txt
